/**
 * CIPHER Auth v3 - GLOBAL BİLEŞEN
 * DB ve Auth nesnelerini pencereye (window) bağlayarak 
 * "is not defined" hatasını kesin olarak çözer.
 */
...
window.Auth = (() => {
    ...
    const hashPassword = (pass) => Promise.resolve(_sha256(pass + '_cipher_salt'));

    // --- REGISTER FONKSİYONU (HATAYI DÜZELTEN KISIM) ---
    async function register(username, password, name) {
        if (!username || !password) throw new Error("Bilgileri doldurun.");
        const uname = username.toLowerCase().trim();
        
        const exists = await window.DB.getUser(uname);
        if (exists) throw new Error("Bu kullanıcı zaten var.");
        ...
        return await window.DB.saveUser(newUser);
    }

    async function login(username, password) {
        const uname = (username || "").toLowerCase().trim();
        const user = await window.DB.getUser(uname);
        if (!user) throw new Error("Kullanıcı bulunamadı.");
        const hash = await hashPassword(password);
        if (user.password_hash !== hash) throw new Error("Şifre yanlış.");
        localStorage.setItem(SK, JSON.stringify({username: uname, expires: Date.now() + 86400000}));
        return user;
    }

    return { register, login, hashPassword };
})();

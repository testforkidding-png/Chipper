/**
 * CIPHER Auth v5
 * localStorage session, pure-JS SHA-256, register, forgot password
 */
const Auth = (() => {
  ...
  const hashPassword = str => Promise.resolve(_sha256(str+'_cipher_salt'));

  // ── Encryption (HTTPS only) ───────────────────────────────────
  async function deriveEncKey(pass,uid){ if(typeof crypto==='undefined'||!crypto.subtle)return; try{ const km=await crypto.subtle.importKey('raw',new TextEncoder().encode(pass),'PBKDF2',false,['deriveKey']); _encKey=await crypto.subtle.deriveKey({name:'PBKDF2',salt:new TextEncoder().encode('cipher_'+uid),iterations:100000,hash:'SHA-256'},km,{name:'AES-GCM',length:256},false,['encrypt','decrypt']); }catch{_encKey=null;} }
  ...
  // ── Session ───────────────────────────────────────────────────
  function saveSession(user){ localStorage.setItem(SK,JSON.stringify({username:user.username,expires:Date.now()+CONFIG.SESSION_TIMEOUT_HOURS*3600000})); }
  function getSession(){ try{ const s=JSON.parse(localStorage.getItem(SK)); if(!s||s.expires<Date.now()){clearSession();return null;} return s; }catch{return null;} }
  ...
  async function login(username, password) {
    ...
    saveSession(user);
    // Update last_seen
    await DB.updateUser(uname, { last_seen: Date.now(), online: true });
    return user;
  }
  ...
  async function register(username, password, displayName) {
    ...
    if (!/^[a-z0-9_.-]{3,20}$/.test(uname)) throw new Error('Kullanıcı adı: 3-20 karakter, harf/rakam/_');
    if (password.length < 6) throw new Error('Şifre en az 6 karakter.');
    if (displayName.trim().length < 2) throw new Error('Ad en az 2 karakter.');
  ...
  async function changeDisplayName(username, newName) {
    if (!newName || newName.trim().length < 2) throw new Error('Ad en az 2 karakter.');
    await DB.updateUser(username, { display_name: newName.trim() });
  }
  ...
  async function changePassword(username, oldPassword, newPassword) {
    ...
    if (user.password_hash !== oldHash) throw new Error('Mevcut şifre yanlış.');
    if (newPassword.length < 6) throw new Error('Yeni şifre en az 6 karakter.');
    const newHash = await hashPassword(newPassword);
    await DB.updateUser(username, { password_hash: newHash });
  }

  // ── Forgot password (admin reset code) ───────────────────────
  async function resetPasswordWithCode(username, code, newPassword) {
    // code = admin key (simple but functional)
    if (code !== CONFIG.ADMIN_KEY) throw new Error('Kod yanlış. Admin panelinden alın.');
    if (newPassword.length < 6) throw new Error('Şifre en az 6 karakter.');
    const user = await DB.getUser(username.toLowerCase().trim());
    if (!user) throw new Error('Kullanıcı bulunamadı.');
    const hash = await hashPassword(newPassword);
    await DB.updateUser(user.username, { password_hash: hash, stale_hash: false });
    return true;
  }
  ...
  function startHeartbeat(username) {
    const tick = async () => {
      await DB.updateUser(username, { last_seen: Date.now(), online: true });
    };
    tick();
    const iv = setInterval(tick, 30000); // every 30s
    // Mark offline on unload
    window.addEventListener('beforeunload', () => {
      DB.updateUser(username, { last_seen: Date.now(), online: false });
    });
    document.addEventListener('visibilitychange', () => {
      if (document.hidden) DB.updateUser(username, { last_seen: Date.now(), online: false });
      else tick();
    });
    return iv;
  }

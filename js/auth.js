...
const Auth = (() => {
  ...
  // Encryption (HTTPS only, gracefully disabled on HTTP)
  ...
  // Session in localStorage (NOT sessionStorage — iOS Safari wipes sessionStorage)
  ...
  async function login(username, password) {
    ...
    let user;
    try { user = await DB.getUser(uname); }
  ...
  async function register(username, password, displayName) {
    ...
    let existing;
    try { existing = await DB.getUser(uname); }
    catch(e) { throw new Error('Sunucuya bağlanılamadı. Supabase ayarlarını kontrol edin.'); }
  ...
  function logout() {
    const s = getSession();
    if (s) { try { DB.updateUser(s.username, { last_seen: Date.now(), online: false }); } catch {} }
    clearSession();

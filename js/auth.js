 ...
 * CIPHER Auth v7
 * Fixes:
 *  - Real error messages (no more swallowed errors)
 *  - Session caches user object → boot doesn't need extra DB call
 *  - getSession().user returns cached user for instant boot
 */
const Auth = (() => {
  ...
  function _sha256(str) {
    ...
    const bytes=new TextEncoder().encode(str),len=bytes.length,bitLen=len*8;
  ...
  // ── Session ───────────────────────────────────────────────────
  // Stores a SAFE copy of user object — bootApp reads this for instant render
  function saveSession(user) {
    const safe = {
      ...
      badges:       user.badges       || [],
      banner_color: user.banner_color || '#0A1628',
      status:       user.status       || '',
  ...
  async function login(username, password) {
    ...
    let user;
    try {
      user = await DB.getUser(uname);
    } catch(e) {
      // Show actual error, not a generic message
      const msg = e.message || '';
      if (msg.includes('column') || msg.includes('schema'))
        throw new Error('Veritabanı şeması eksik. Admin panelindeki SQL\'i çalıştırın.');
      if (msg.includes('Failed to fetch') || msg.includes('NetworkError') || msg.includes('fetch'))
        throw new Error('Sunucuya ulaşılamıyor. İnternet bağlantınızı kontrol edin.');
      throw new Error('Giriş hatası: ' + msg);
    }
  ...
  async function register(username, password, displayName) {
    ...
    if (!/^[a-z0-9_.-]{3,20}$/.test(uname)) throw new Error('Kullanıcı adı: 3-20 karakter, küçük harf/rakam/_');
    ...
    let existing;
    try {
      existing = await DB.getUser(uname);
    } catch(e) {
      const msg = e.message || '';
      if (msg.includes('column') || msg.includes('schema'))
        throw new Error('Veritabanı şeması eksik. Admin panelindeki SQL\'i çalıştırın.');
      if (msg.includes('Failed to fetch') || msg.includes('NetworkError'))
        throw new Error('Sunucuya ulaşılamıyor. İnternet bağlantınızı kontrol edin.');
      throw new Error('Kayıt hatası: ' + msg);
    }
    ...
    try {
      user = await DB.createUser({
        ...
        last_seen: now, online: true, created_at: now,
      });
    } catch(e) {
      throw new Error('Kayıt başarısız: ' + (e.message || e));
    }
  ...
  async function changeDisplayName(username, newName) {
    ...
    return await DB.updateUser(username, { display_name: newName.trim() });
  }
  ...
  // currentUser: use session cache first, DB second
  async function currentUser() {
    const s = getSession();
    if (!s) return null;
    if (s.user) return s.user; // from cache
  ...
  function _saveLsLocal(u, ts, online) { try { localStorage.setItem(_lsKey(u), JSON.stringify({ts, online})); } catch {} }
  function _getLsLocal(u) { try { return JSON.parse(localStorage.getItem(_lsKey(u))); } catch { return null; } }

const UI = (() => {
  ...
  function showCtxMenu(x, y, items) {
    ...
    const safeY=Math.min(y, window.innerHeight-items.length*40-10);
  ...
  function showProfileCard(user, anchorEl) {
    ...
    const badges=(user.badges||[]).map(b=>{ const bd=CONFIG.BADGES[b]; return bd?`<span title="${bd.label}" style="background:${bd.color}22;border:1px solid ${bd.color}44;color:${bd.color};padding:2px 8px;border-radius:8px;font-size:11px">${bd.icon} ${bd.label}</span>`:''; }).join('');
    const avHtml=user.avatar_url
      ?`<img src="${user.avatar_url}" style="width:64px;height:64px;border-radius:50%;object-fit:cover;border:3px solid #0C1220">`
    ...
    card.innerHTML=`<div style="height:72px;background:${banner};position:relative"><div style="position:absolute;bottom:-32px;left:14px">${avHtml}</div><div style="position:absolute;top:8px;right:10px;width:11px;height:11px;background:#00E676;border-radius:50%;border:2px solid #0C1220"></div></div>
    <div style="padding:38px 14px 14px">
      <div style="display:flex;align-items:start;justify-content:space-between;margin-bottom:3px">
        <div><div style="font-family:Syne,sans-serif;font-weight:700;font-size:16px;color:#DDE8F8">${user.display_name||user.username}</div><div style="font-size:11px;color:#7A8FA8;font-family:'JetBrains Mono',monospace">@${user.username}</div></div>
        ${user.is_admin?'<span style="font-size:10px;padding:2px 7px;border-radius:20px;background:#FFD70022;color:#FFD700;border:1px solid #FFD70044">ADMİN</span>':''}
      </div>
      ${user.status?`<div style="font-size:12px;color:#B0C4D8;margin:6px 0">${user.status_emoji||''} ${user.status}</div>`:''}
      <div style="height:1px;background:#1E2D45;margin:8px 0"></div>
      ${user.bio?`<div style="font-size:12px;color:#9AB0C8;margin-bottom:8px;line-height:1.5">${user.bio}</div>`:''}
      <div style="font-size:11px;color:#7A8FA8;font-family:'JetBrains Mono',monospace;margin-bottom:4px">CIPHER ÜYESİ</div>
      <div style="font-size:12px;color:#DDE8F8;margin-bottom:${badges?'10':'0'}px">${fmtDate(user.created_at||Date.now())}</div>
    ...
    if(anchorEl){
      ...
      card.style.top=top+'px'; card.style.left=Math.max(8,left)+'px';
    } else {
  ...
  function showReactionPicker(x, y, onPick) {
    ...
    const safeX=Math.min(x-130, window.innerWidth-270);
    const safeY=Math.min(y-180, window.innerHeight-220);
    p.style.left=Math.max(4,safeX)+'px';
    p.style.top=Math.max(4,safeY)+'px';
